...
RSpec.describe "Members", type: :request do
  ...
  describe ':: Members / Same Family ::' do
    ...
    context "GET /members Members#create" do
      it "works! (now write some real specs)" do
        get '/v1/members'
        json = JSON.parse(response.body) 
        header = JSON.parse(response.header)
    ...
    context "GET /members Members#search" do
      it "works! (now write some real specs)" do
        get '/v1/members'
        json = JSON.parse(response.body) 
        header = JSON.parse(response.header)
  ...
  describe ':: Members / Same Family - Admin Role ::' do
    ...
    context "GET /members Members#update" do
      it "able to #put update another family member's profile" do
        get '/v1/members'
        json = JSON.parse(response.body) 
        header = JSON.parse(response.header)
        expect(response).to have_http_status(200)
      end
      it "able to #patch update another family member's profile" do
        get '/v1/members'
        json = JSON.parse(response.body) 
        header = JSON.parse(response.header)
        expect(response).to have_http_status(200)
      end
      it "able to make a family member an admin" do
        get '/v1/members'
        json = JSON.parse(response.body) 
        header = JSON.parse(response.header)
        expect(response).to have_http_status(200)
      end
      it "able to remove a family member's status as admin" do
        get '/v1/members'
        json = JSON.parse(response.body) 
        header = JSON.parse(response.header)
        expect(response).to have_http_status(200)
